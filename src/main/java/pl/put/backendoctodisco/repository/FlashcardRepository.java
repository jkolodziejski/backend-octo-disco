...
public interface FlashcardRepository extends JpaRepository<Flashcard, Integer> {
    ...
    @Query(countQuery = "SELECT  count(DISTINCT f.id) from flashcard f join alias a on f.id = a.word_id where word like concat('%',?1,'%') or alias like  concat('%',?1,'%') ",
            value = "SELECT DISTINCT f.* from flashcard f join alias a on f.id = a.word_id where word like concat('%',?1,'%') or alias like  concat('%',?1,'%') ",
            nativeQuery = true)
